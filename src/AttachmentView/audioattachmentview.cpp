...
const double AudioAttachmentView::SLIDER_MULTIPLIER = 100;
    ...
    ui(new Ui::AudioAttachmentView)
{
    ...
    connect( ui->doubleSpinBoxTempo,     SIGNAL(editingFinished()), this, SLOT(on_doubleSpinBoxTempo_editingFinished())     );
    connect( ui->doubleSpinBoxPitch,     SIGNAL(editingFinished()), this, SLOT(on_doubleSpinBoxPitch_editingFinished())     );
...
void AudioAttachmentView::polish()
{
    ...
    connect( &a->player(),  SIGNAL(notify()), this, SLOT(updateSliders()) );
...
void AudioAttachmentView::on_doubleSpinBoxTempo_editingFinished()
{
    player().setStretch( ui->doubleSpinBoxTempo->value() );
}

void AudioAttachmentView::on_doubleSpinBoxPitch_editingFinished()
{
    player().setPitch( ui->doubleSpinBoxPitch->value() );
}
...
{
    double elapsed = player().elapsedSec();
